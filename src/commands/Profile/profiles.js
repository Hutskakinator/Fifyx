...
module.exports = {
    ...
    .addSubcommand(command => command.setName('create').setDescription('Creates a user profile.')
        ...
        .addStringOption(option => option.setName('birthday').setDescription('Your birthday').setRequired(false))
    ...
    .addSubcommand(command => command.setName('edit').setDescription('Edits your user profile.')
    ...
    .addSubcommand(command => command.setName('view').setDescription('Views a user profile.')
    ...
    async execute(interaction, client) {
        ...
        switch (sub) {
            case 'create': {
                ...
                if (birthdayString) {
                    const formats = ['DD/MM/YYYY', 'YYYY/DD/MM', 'DD MMMM YYYY', 'YYYY-MM-DD', 'YYYY/MM/DD', 'DD.MM.YYYY'];
                    birthday = moment(birthdayString, formats, true);  // Parse using multiple formats

                    if (!birthday.isValid()) {
                        await interaction.reply({ content: 'Invalid date format. Please use a valid format like DD/MM/YYYY, YYYY/MM/DD, or DD MMMM YYYY.', ephemeral: true });
                        return;
                    }
                }
                ...
                if (birthday) newProfileData.birthday = birthday.toDate();
            ...
            case 'edit': {
                ...
                if (birthdayInput) {
                    const formats = ['DD/MM/YYYY', 'YYYY/DD/MM', 'DD MMMM YYYY', 'YYYY-MM-DD', 'YYYY/MM/DD', 'DD.MM.YYYY'];
                    const birthday = moment(birthdayInput, formats, true);  // Parse using multiple formats

                    if (!birthday.isValid()) {
                        await interaction.reply({ content: 'Invalid birthday format. Please use a valid format like DD/MM/YYYY, YYYY/MM/DD, or DD MMMM YYYY.', ephemeral: true });
                        return;
                    }
